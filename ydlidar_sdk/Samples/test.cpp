...
void LaserScanCallback(const LaserScan &scan)
{
    ...
    for (size_t i = 0; i < scan.ranges.size(); i++)
    {
        ...
        double current_angle = scan.config.min_angle + i * scan.config.ang_increment; // radian format
        double angle = current_angle * 180 / M_PI;                                    //Angle fomat
   ...
   // std::cout << "\b" << "front = " << front << " | back = " << back << " | babord = " << babord << " | tribord = " << tribord << std::endl;
        //UNUSED(angle);
        UNUSED(distance);
        UNUSED(intensity);
    }
    stringstream stream, stream2, stream3, stream4;
    stream << fixed << std::setprecision(4) << front;
    stream2 << fixed << std::setprecision(4) << back;
    stream3 << fixed << std::setprecision(4) << babord;
    stream4 << fixed << std::setprecision(4) << tribord;
    string frontStr = stream.str();
    string backStr = stream2.str();
    string babordStr = stream3.str();
    string tribordStr = stream4.str();
